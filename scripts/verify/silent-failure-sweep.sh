...
# mapping can disagree and nothing reports it: fstrim exits non-zero, no timer covers it,
# and the host looks healthy while reclaiming nothing. Four unprivileged reads name the
# state honestly, so STAGED is never mistaken for ACTIVE and an untrimmed root is never
# mistaken for a healthy one:
#   /proc/cmdline                                what this boot actually declared
#   /etc/default/limine                          what the next boot will declare
...
file=no; grep -q allow-discards /etc/default/limine 2>/dev/null && file=yes
echo "host=$(hostname) gran=$gran decl=$decl file=$file"
SNIP
)
  h="${out%% *}"; h="${h#host=}"
  case "$out" in
    ...
    *gran=0' '*decl=yes*)
                       emit FAIL D10 trim-declared-not-applied "$h/root" "${out#* }" "the boot declared discards but dm got none: the token never reached cryptsetup" ;;
    *gran=0' '*file=yes*)
                       emit NOTE D10 trim-staged "$h/root" "${out#* }" "declared for the next boot, not active yet — scheduled, not broken" ;;
    *gran=0' '*)
      if [ "$h" = zephyr ]; then
