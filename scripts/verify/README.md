...
| `media_restore_drill.sh` | That the media metadata backup exists, is fresh (newest object < 48 h), has **ever** verified itself, and carries no stale `*.db-wal` / `*.db-shm` sidecars; that the metadata tier and the library are different devices **and that the backup is not stored on the library's device**; the live *arr/Jellyfin row counts against floors; a scratch restore that is actually opened — per-database `integrity_check` plus row counts compared against live, with the measured restore time; the library's measured size per directory, `downloads/` excluded, and the absence of any backup for it; and that its own scratch directory is deleted and proven deleted. | nexus, any cwd | yes (the only writes are inside one scratch dir on `/data/nvme1`, deleted on exit) | `the metadata backup is stored on the SAME device as the library`, `media-config-backup has NEVER logged its own success line`, `N *.db-wal / *.db-shm object(s) sit in the bucket`, `NO garage bucket corresponds to any library directory` (all five are the correct current state — see `docs/MEDIA-RESTORE-REHEARSAL.md`) |
...
# the media library + metadata restore rehearsal (~30 s bounded; '--full' rehearses all 22.8k objects)
ssh nexus 'bash ~/homelab-ops/scripts/verify/media_restore_drill.sh'
